...
void plotChar(enum displayGlyph inputChar, short xLoc, short yLoc,
              short foreRed, short foreGreen, short foreBlue, short backRed,
              short backGreen, short backBlue) {
  currentConsole.plotChar(inputChar, xLoc, yLoc, foreRed, foreGreen, foreBlue,
...
void textInputStart() { currentConsole.textInputStart(); }

void textInputStop() { currentConsole.textInputStop(); }
...
// sorts the entries of the scoreBuffer global variable by score in descending
// order; returns the sorted line number of the most recent entry
...
short loadScoreBuffer() {
  ...
  for (i = 0; i < HIGH_SCORES_COUNT; i++) {
    ...
    fscanf(scoresFile, "%li\t%li\t", &(scoreBuffer[i].score),
           &(scoreBuffer[i].dateNumber));
...
// The numerical version of the date is what gets saved; the "mm/dd/yy" version
// is ignored. Does NOT do any sorting.
void saveScoreBuffer() {
  ...
  for (i = 0; i < HIGH_SCORES_COUNT; i++) {
    ...
    fprintf(scoresFile, "%li\t%li\t%s\n", scoreBuffer[i].score,
            scoreBuffer[i].dateNumber, scoreBuffer[i].description);
  }
...
fileEntry *addfile(struct filelist *list, const char *name) {
  ...
  list->files[list->nfiles].path =
      ((char *)NULL) +
      list->nextname; // don't look at them until they are transferred out
  list->files[list->nfiles].date = (struct tm){
      0}; // associate a dummy date (1899-12-31) to avoid random data, it will
          // be correctly populated when using listFiles()
...
fileEntry *commitFilelist(struct filelist *list, char **namebuffer) {
  int i;
  /*fileEntry *files = malloc(list->nfiles * sizeof(fileEntry) +
  list->nextname); // enough space for all the names and all the files
