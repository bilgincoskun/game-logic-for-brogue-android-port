...
#define MONSTERS_ENABLED                                                       \
  (!rogue.wizard || 1) // Quest room monsters can be generated regardless.
...
//#define BROGUE_ASSERTS        // introduces several assert()s -- useful to
// find certain array overruns and other bugs #define AUDIT_RNG             //
// VERY slow, but sometimes necessary to debug out-of-sync recording errors
//#define GENERATE_FONT_FILES   // Displays font in grid upon startup, which can
// be screen-captured into font files for PC.
...
#define FP_BASE                                                                \
  16 // Don't change this without recalculating all of the power tables
     // throughout the code!
...
// Size of the entire terminal window. These need to be hard-coded here and in
// Viewport.h
#define COLS 100
...
#define DCOLS                                                                  \
  (COLS - STAT_BAR_WIDTH -                                                     \
   1) // n columns on the left for the sidebar;
...
#define STAT_BAR_WIDTH                                                         \
  20 // number of characters in the stats bar to the left of the map
...
#define LIGHT_SMOOTHING_THRESHOLD                                              \
  150 // light components higher than this magnitude will be toned down a little

#define MAX_BOLT_LENGTH DCOLS * 10

#define VISIBILITY_THRESHOLD                                                   \
  50 // how bright cumulative light has to be before the cell is marked visible
...
#define INPUT_RECORD_BUFFER                                                    \
  1000 // how many bytes of input data to keep in memory before saving it to
       // disk
...
enum itemCategory {
  ...
  CAN_BE_DETECTED =
      (WEAPON | ARMOR | POTION | SCROLL | RING | CHARM | WAND | STAFF | AMULET),
  ...
  ALL_ITEMS = (FOOD | POTION | WEAPON | ARMOR | STAFF | WAND | SCROLL | RING |
               CHARM | GOLD | AMULET | GEM | KEY),
};

enum keyKind { KEY_DOOR, KEY_CAGE, KEY_PORTAL, NUMBER_KEY_TYPES };
...
enum tileFlags {
  DISCOVERED = Fl(0),
  VISIBLE = Fl(
      1), // cell has sufficient light and is in field of view, ready to draw.
  HAS_PLAYER = Fl(2),
  HAS_MONSTER = Fl(3),
  HAS_DORMANT_MONSTER = Fl(4), // hidden monster on the square
  HAS_ITEM = Fl(5),
  IN_FIELD_OF_VIEW = Fl(6), // player has unobstructed line of sight whether or
                            // not there is enough light
  WAS_VISIBLE = Fl(7),
  HAS_STAIRS = Fl(8),
  SEARCHED_FROM_HERE =
      Fl(9), // player already auto-searched here; can't auto-search here again
  ...
  CLAIRVOYANT_DARKENED =
      Fl(15), // magical blindness from a cursed ring of clairvoyance
  CAUGHT_FIRE_THIS_TURN = Fl(16), // so that fire does not spread asymmetrically
  PRESSURE_PLATE_DEPRESSED =
      Fl(17), // so that traps do not trigger repeatedly while you stand on them
  STABLE_MEMORY =
      Fl(18), // redraws will be pulled from the memory array, not recalculated
  KNOWN_TO_BE_TRAP_FREE =
      Fl(19), // keep track of where the player has stepped or watched monsters
              // step as he knows no traps are there
  IS_IN_PATH = Fl(20),    // the yellow trail leading to the cursor
  IN_LOOP = Fl(21),       // this cell is part of a terrain loop
  IS_CHOKEPOINT = Fl(22), // if this cell is blocked, part of the map will be
                          // rendered inaccessible
  IS_GATE_SITE = Fl(23),  // consider placing a locked door here
  IS_IN_ROOM_MACHINE = Fl(24),
  IS_IN_AREA_MACHINE = Fl(25),
  IS_POWERED = Fl(26),  // has been activated by machine power this turn (flag
                        // can probably be eliminated if needed)
  IMPREGNABLE = Fl(27), // no tunneling allowed!
  TERRAIN_COLORS_DANCING =
      Fl(28), // colors here will sparkle when the game is idle
  TELEPATHIC_VISIBLE =
      Fl(29), // potions of telepathy let you see through other creatures' eyes
  WAS_TELEPATHIC_VISIBLE =
      Fl(30), // potions of telepathy let you see through other creatures' eyes

  IS_IN_MACHINE = (IS_IN_ROOM_MACHINE |
                   IS_IN_AREA_MACHINE), // sacred ground; don't generate items
                                        // here, or teleport randomly to it

  PERMANENT_TILE_FLAGS =
      (DISCOVERED | MAGIC_MAPPED | ITEM_DETECTED | HAS_ITEM |
       HAS_DORMANT_MONSTER | HAS_MONSTER | HAS_STAIRS | SEARCHED_FROM_HERE |
       PRESSURE_PLATE_DEPRESSED | STABLE_MEMORY | KNOWN_TO_BE_TRAP_FREE |
       IN_LOOP | IS_CHOKEPOINT | IS_GATE_SITE | IS_IN_MACHINE | IMPREGNABLE),
...
#define XPXP_NEEDED_FOR_TELEPATHIC_BOND                                        \
  1400 // XPXP required to enable telepathic awareness with the ally
...
// Making these larger means cave generation will take more trials; set them too
// high and the program will hang.
...
#define clamp(x, low, hi)                                                      \
  (min(hi, max(x, low))) // pins x to the [y, z] interval
...
#else
#define cellHasTerrainFlag(x, y, flagMask)                                     \
  ((flagMask)&terrainFlags((x), (y)) ? true : false)
#endif
#define cellHasTMFlag(x, y, flagMask)                                          \
  ((flagMask)&terrainMechFlags((x), (y)) ? true : false)
...
#define cellHasKnownTerrainFlag(x, y, flagMask)                                \
  ((flagMask)&pmap[(x)][(y)].rememberedTerrainFlags ? true : false)
...
#define coordinatesAreInMap(x, y)                                              \
  ((x) >= 0 && (x) < DCOLS && (y) >= 0 && (y) < DROWS)
#define coordinatesAreInWindow(x, y)                                           \
...
#define CYCLE_MONSTERS_AND_PLAYERS(x)                                          \
  for ((x) = &player; (x) != NULL;                                             \
       (x) = ((x) == &player ? monsters->nextCreature : (x)->nextCreature))
...
// weighted sum of the squares of the component differences. Weights are
// according to color perception.
...
typedef struct pcell { // permanent cell; have to remember this stuff to save
                       // levels
  ...
  enum itemCategory rememberedItemCategory; // what category of item the player
                                            // remembers lying there
  short
      rememberedItemKind; // what kind of item the player remembers lying there
  short rememberedItemQuantity;    // how many of the item the player remembers
                                   // lying there
  short rememberedItemOriginDepth; // the origin depth of the item the player
                                   // remembers lying there
  enum tileType
      rememberedTerrain; // what the player remembers as the terrain (i.e.
                         // highest priority terrain upon last seeing)
  unsigned long
      rememberedCellFlags; // map cell flags the player remembers from that spot
  unsigned long rememberedTerrainFlags; // terrain flags the player remembers
                                        // from that spot
  unsigned long
      rememberedTMFlags; // TM flags the player remembers from that spot
} pcell;

typedef struct tcell { // transient cell; stuff we don't need to remember
                       // between levels
  short light[3];      // RGB components of lighting
  short oldLight[3]; // compare with subsequent lighting to determine whether to
                     // refresh cell
} tcell;
...
enum itemFlags {
  ...
  ITEM_KIND_AUTO_ID =
      Fl(22), // the item type will become known when the item is picked up.
  ITEM_PLAYER_AVOIDS =
      Fl(23), // explore and travel will try to avoid picking the item up
};
...
enum dungeonFeatureTypes {
  ...
  // statues crack for a few turns and then shatter, revealing the monster
  // inside
  DF_CRACKING_STATUE,
  DF_STATUE_SHATTER,
...
typedef struct flare {
  ...
  short coeffChangeAmount; // The constant amount by which the coefficient
                           // changes per frame, e.g. -25 means it gets 25%
                           // dimmer per frame.
  short coeffLimit; // Flare ends if the coefficient passes this percentage
                    // (whether going up or down).
  short xLoc, yLoc; // Current flare location.
  long coeff;       // Current flare coefficient; always starts at 100.
  unsigned long
      turnNumber; // So we can eliminate those that fired one or more turns ago.
} flare;

enum DFFlags {
  DFF_EVACUATE_CREATURES_FIRST =
      Fl(0), // Creatures in the DF area get moved outside of it
  DFF_SUBSEQ_EVERYWHERE = Fl(1), // Subsequent DF spawns in every cell that this
                                 // DF spawns in, instead of only the origin
  DFF_TREAT_AS_BLOCKING =
      Fl(2), // If filling the footprint of this DF with walls would disrupt
             // level connectivity, then abort.
  DFF_PERMIT_BLOCKING =
      Fl(3), // Generate this DF without regard to level connectivity.
  DFF_ACTIVATE_DORMANT_MONSTER =
      Fl(4), // Dormant monsters on this tile will appear -- e.g. when a statue
             // bursts to reveal a monster.
  DFF_CLEAR_OTHER_TERRAIN =
      Fl(5), // Erase other terrain in the footprint of this DF.
  DFF_BLOCKED_BY_OTHER_LAYERS =
      Fl(6), // Will not propagate into a cell if any layer in that cell has a
             // superior priority.
  DFF_SUPERPRIORITY = Fl(7), // Will overwrite terrain of a superior priority.
  DFF_AGGRAVATES_MONSTERS =
      Fl(8), // Will act as though an aggravate monster scroll of effectRadius
             // radius had been read at that point.
  DFF_RESURRECT_ALLY =
      Fl(9), // Will bring back to life your most recently deceased ally.
};
...
enum boltFlags {
  BF_PASSES_THRU_CREATURES =
      Fl(0), // Bolt continues through creatures (e.g. lightning and tunneling)
  BF_HALTS_BEFORE_OBSTRUCTION =
      Fl(1), // Bolt takes effect the space before it terminates (e.g.
             // conjuration, obstruction, blinking)
  BF_TARGET_ALLIES = Fl(2),  // Staffs/wands/creatures that shoot this bolt will
                             // auto-target allies.
  BF_TARGET_ENEMIES = Fl(3), // Staffs/wands/creatures that shoot this bolt will
                             // auto-target enemies.
  BF_FIERY = Fl(4), // Bolt will light flammable terrain on fire as it passes,
                    // and will ignite monsters hit.
  BF_NEVER_REFLECTS =
      Fl(6), // Bolt will never reflect (e.g. spiderweb, arrows).
  BF_NOT_LEARNABLE =
      Fl(7), // This technique cannot be absorbed by empowered allies.
  ...
  BF_DISPLAY_CHAR_ALONG_LENGTH =
      Fl(10), // Display the character along the entire length of the bolt
              // instead of just at the front.
};
...
typedef struct dungeonProfile {
  // Room type weights (in the natural dungeon, these are also adjusted based on
  // depth):
...
typedef struct floorTileType {
  ...
  const color *foreColor;
  const color *backColor;
  short drawPriority;  // priority (lower number means higher priority); governs
                       // drawing as well as tile replacement comparisons.
  char chanceToIgnite; // chance to burn if a flame terrain is on one of the
                       // four cardinal neighbors
  enum dungeonFeatureTypes
      fireType; // spawn this DF when the terrain ignites (or, if it's
                // T_IS_DF_TRAP, when the pressure plate clicks)
  enum dungeonFeatureTypes discoverType; // spawn this DF when successfully
                                         // searched if T_IS_SECRET is set
  enum dungeonFeatureTypes
      promoteType; // creates this dungeon spawn type when it promotes for some
                   // other reason (random promotion or promotion through
                   // machine activation)
  short promoteChance; // percent chance per turn to spawn the promotion type;
                       // will also vanish upon doing so if
...
enum terrainFlagCatalog {
  ...
  T_OBSTRUCTS_SURFACE_EFFECTS =
      Fl(3),               // grass, blood, etc. cannot exist on this tile
  T_OBSTRUCTS_GAS = Fl(4), // blocks the permeation of gas
  T_OBSTRUCTS_DIAGONAL_MOVEMENT =
      Fl(5), // can't step diagonally around this tile
  T_SPONTANEOUSLY_IGNITES =
      Fl(6), // monsters avoid unless chasing player or immune to fire
  T_AUTO_DESCENT = Fl(7), // automatically drops creatures down a depth level
                          // and does some damage (2d6)
  T_LAVA_INSTA_DEATH =
      Fl(8), // kills any non-levitating non-fire-immune creature instantly
  T_CAUSES_POISON = Fl(9), // any non-levitating creature gets 10 poison
  T_IS_FLAMMABLE = Fl(10), // terrain can catch fire
  T_IS_FIRE =
      Fl(11), // terrain is a type of fire; ignites neighboring flammable cells
  T_ENTANGLES = Fl(12), // entangles players and monsters like a spiderweb
  T_IS_DEEP_WATER =
      Fl(13), // steals items 50% of the time and moves them around randomly
  T_CAUSES_DAMAGE =
      Fl(14), // anything on the tile takes max(1-2, 10%) damage per turn
  ...
  T_CAUSES_CONFUSION =
      Fl(17), // causes creatures on this tile to become confused
  T_CAUSES_HEALING = Fl(
      18), // heals 20% max HP per turn for any player or non-inanimate monsters
  T_IS_DF_TRAP =
      Fl(19), // spews gas of type specified in fireType when stepped on
  T_CAUSES_EXPLOSIVE_DAMAGE =
      Fl(20), // is an explosion; deals higher of 15-20 or 50% damage instantly,
              // but not again for five turns
  T_SACRED = Fl(
      21), // monsters that aren't allies of the player will avoid stepping here
  ...
  T_PATHING_BLOCKER = (T_OBSTRUCTS_PASSABILITY | T_AUTO_DESCENT | T_IS_DF_TRAP |
                       ...
                       T_SPONTANEOUSLY_IGNITES),
  T_DIVIDES_LEVEL = (T_OBSTRUCTS_PASSABILITY | T_AUTO_DESCENT | T_IS_DF_TRAP |
                     T_LAVA_INSTA_DEATH | T_IS_DEEP_WATER),
  T_LAKE_PATHING_BLOCKER = (T_AUTO_DESCENT | T_LAVA_INSTA_DEATH |
  ...
  T_HARMFUL_TERRAIN =
      (T_CAUSES_POISON | T_IS_FIRE | T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS |
       T_CAUSES_CONFUSION | T_CAUSES_EXPLOSIVE_DAMAGE),
  T_RESPIRATION_IMMUNITIES = (T_CAUSES_DAMAGE | T_CAUSES_CONFUSION |
                              T_CAUSES_PARALYSIS | T_CAUSES_NAUSEA),
};

enum terrainMechanicalFlagCatalog {
  TM_IS_SECRET = Fl(0), // successful search or being stepped on while visible
                        // transforms it into discoverType
  TM_PROMOTES_WITH_KEY =
      Fl(1), // promotes if the key is present on the tile (in your pack,
             // carried by monster, or lying on the ground)
  TM_PROMOTES_WITHOUT_KEY =
      Fl(2), // promotes if the key is NOT present on the tile (in your pack,
             // carried by monster, or lying on the ground)
  TM_PROMOTES_ON_STEP = Fl(3), // promotes when a creature, player or item is on
                               // the tile (whether or not levitating)
  TM_PROMOTES_ON_ITEM_PICKUP = Fl(4),  // promotes when an item is lifted from
                                       // the tile (primarily for altars)
  TM_PROMOTES_ON_PLAYER_ENTRY = Fl(5), // promotes when the player enters the
                                       // tile (whether or not levitating)
  TM_PROMOTES_ON_SACRIFICE_ENTRY =
      Fl(6), // promotes when the sacrifice target enters the tile (whether or
             // not levitating)
  TM_PROMOTES_ON_ELECTRICITY = Fl(7), // promotes when hit by a lightning bolt
  TM_ALLOWS_SUBMERGING =
      Fl(8), // allows submersible monsters to submerge in this terrain
  TM_IS_WIRED =
      Fl(9), // if wired, promotes when powered, and sends power when promoting
  TM_IS_CIRCUIT_BREAKER =
      Fl(10),                 // prevents power from circulating in its machine
  ...
  TM_VANISHES_UPON_PROMOTION =
      Fl(14), // vanishes when creating promotion dungeon feature, even if the
              // replacement terrain priority doesn't require it
  TM_REFLECTS_BOLTS = Fl(15), // magic bolts reflect off of its surface randomly
                              // (similar to pmap flag IMPREGNABLE)
  TM_STAND_IN_TILE = Fl(16),  // earthbound creatures will be said to stand "in"
                              // the tile, not on it
  TM_LIST_IN_SIDEBAR = Fl(17), // terrain will be listed in the sidebar with a
                               // description of the terrain type
  TM_VISUALLY_DISTINCT =
      Fl(18), // terrain will be color-adjusted if necessary so the character
              // stands out from the background
  TM_BRIGHT_MEMORY = Fl(19), // no blue fade when this tile is out of sight
  TM_EXPLOSIVE_PROMOTE =
      Fl(20), // when burned, will promote to promoteType instead of burningType
              // if surrounded by tiles with T_IS_FIRE or TM_EXPLOSIVE_PROMOTE
  TM_CONNECTS_LEVEL = Fl(
      21), // will be treated as passable for purposes of calculating level
           // connectedness, irrespective of other aspects of this terrain layer
  TM_INTERRUPT_EXPLORATION_WHEN_SEEN =
      Fl(22), // will generate a message when discovered during exploration to
              // interrupt exploration
  TM_INVERT_WHEN_HIGHLIGHTED =
      Fl(23), // will flip fore and back colors when highlighted with pathing
  TM_SWAP_ENCHANTS_ACTIVATION =
      Fl(24), // in machine, swap item enchantments when two suitable items are
              // on this terrain, and activate the machine when that happens
};
...
enum hordeFlags {
  HORDE_DIES_ON_LEADER_DEATH = Fl(0), // if the leader dies, the horde will die
                                      // instead of electing new leader
  HORDE_IS_SUMMONED = Fl(1), // minions summoned when any creature is the same
                             // species as the leader and casts summon
  HORDE_SUMMONED_AT_DISTANCE =
      Fl(2), // summons will appear across the level, and will naturally path
             // back to the leader
  HORDE_LEADER_CAPTIVE =
      Fl(3), // the leader is in chains and the followers are guards
  HORDE_NO_PERIODIC_SPAWN =
      Fl(4), // can spawn only when the level begins -- not afterwards
  HORDE_ALLIED_WITH_PLAYER = Fl(5),

  HORDE_MACHINE_BOSS = Fl(6), // used in machines for a boss challenge
  HORDE_MACHINE_WATER_MONSTER =
      Fl(7), // used in machines where the room floods with shallow water
  HORDE_MACHINE_CAPTIVE =
      Fl(8), // powerful captive monsters without any captors
  HORDE_MACHINE_STATUE =
      Fl(9), // the kinds of monsters that make sense in a statue
  ...
  HORDE_VAMPIRE_FODDER =
      Fl(13), // monsters that are prone to capture and farming by vampires
  HORDE_MACHINE_LEGENDARY_ALLY = Fl(14), // legendary allies
  HORDE_NEVER_OOD = Fl(15), // Horde cannot be generated out of depth
  HORDE_MACHINE_THIEF =
      Fl(16), // monsters that can be generated in the key thief area machines
  HORDE_MACHINE_GOBLIN_WARREN = Fl(17), // can spawn in goblin warrens
  HORDE_SACRIFICE_TARGET = Fl(18),      // can be the target of an assassination
                                   // challenge; leader will get scary light.

  HORDE_MACHINE_ONLY =
      (HORDE_MACHINE_BOSS | HORDE_MACHINE_WATER_MONSTER |
       HORDE_MACHINE_CAPTIVE | HORDE_MACHINE_STATUE | HORDE_MACHINE_TURRET |
       HORDE_MACHINE_MUD | HORDE_MACHINE_KENNEL | HORDE_VAMPIRE_FODDER |
       HORDE_MACHINE_LEGENDARY_ALLY | HORDE_MACHINE_THIEF |
       HORDE_MACHINE_GOBLIN_WARREN | HORDE_SACRIFICE_TARGET),
};

enum monsterBehaviorFlags {
  MONST_INVISIBLE = Fl(0), // monster is invisible
  MONST_INANIMATE = Fl(1), // monster has abbreviated stat bar display and is
  ...
  MONST_FLEES_NEAR_DEATH =
      Fl(6), // monster flees when under 25% health and re-engages when over 75%
  MONST_ATTACKABLE_THRU_WALLS =
      Fl(7), // can be attacked when embedded in a wall
  ...
  MONST_CAST_SPELLS_SLOWLY =
      Fl(13), // takes twice the attack duration to cast a spell
  MONST_IMMUNE_TO_WEBS = Fl(14), // monster passes freely through webs
  MONST_REFLECT_4 = Fl(15), // monster reflects projectiles as though wearing +4
                            // armor of reflection
  MONST_NEVER_SLEEPS = Fl(16), // monster is always awake
  MONST_FIERY =
      Fl(17), // monster carries an aura of flame (but no automatic fire light)
  MONST_INVULNERABLE = Fl(18),    // monster is immune to absolutely everything
  MONST_IMMUNE_TO_WATER = Fl(19), // monster moves at full speed in deep water
                                  // and (if player) doesn't drop items
  MONST_RESTRICTED_TO_LIQUID =
      Fl(20), // monster can move only on tiles that allow submersion
  MONST_SUBMERGES = Fl(21), // monster can submerge in appropriate terrain
  MONST_MAINTAINS_DISTANCE = Fl(
      22), // monster tries to keep a distance of 3 tiles between it and player
  MONST_WILL_NOT_USE_STAIRS =
      Fl(23), // monster won't chase the player between levels
  MONST_DIES_IF_NEGATED =
      Fl(24),          // monster will die if exposed to negation magic
  MONST_MALE = Fl(25), // monster is male (or 50% likely to be male if also has
                       // MONST_FEMALE)
  MONST_FEMALE = Fl(26), // monster is female (or 50% likely to be female if
                         // also has MONST_MALE)
  MONST_NOT_LISTED_IN_SIDEBAR =
      Fl(27), // monster doesn't show up in the sidebar
  MONST_GETS_TURN_ON_ACTIVATION =
      Fl(28), // monster never gets a turn, except when its machine is activated
  MONST_ALWAYS_USE_ABILITY = Fl(29), // monster will never fail to use special
                                     // ability if eligible (no random factor)
  MONST_NO_POLYMORPH = Fl(30), // monster cannot result from a polymorph spell
                               // (liches, phoenixes and Warden of Yendor)

  NEGATABLE_TRAITS =
      (MONST_INVISIBLE | MONST_DEFEND_DEGRADE_WEAPON | MONST_IMMUNE_TO_WEAPONS |
       MONST_FLIES | MONST_FLITS | MONST_IMMUNE_TO_FIRE | MONST_REFLECT_4 |
       MONST_FIERY | MONST_MAINTAINS_DISTANCE),
  MONST_TURRET = (MONST_IMMUNE_TO_WEBS | MONST_NEVER_SLEEPS | MONST_IMMOBILE |
                  MONST_INANIMATE | MONST_ATTACKABLE_THRU_WALLS |
                  MONST_WILL_NOT_USE_STAIRS),
  LEARNABLE_BEHAVIORS =
      (MONST_INVISIBLE | MONST_FLIES | MONST_IMMUNE_TO_FIRE | MONST_REFLECT_4),
  MONST_NEVER_VORPAL_ENEMY =
      (MONST_INANIMATE | MONST_INVULNERABLE | MONST_IMMOBILE |
       MONST_RESTRICTED_TO_LIQUID | MONST_GETS_TURN_ON_ACTIVATION |
       MONST_MAINTAINS_DISTANCE),
  MONST_NEVER_MUTATED =
      (MONST_INVISIBLE | MONST_INANIMATE | MONST_IMMOBILE | MONST_INVULNERABLE),
};
...
enum monsterAbilityFlags {
  ...
  MA_ENTER_SUMMONS = Fl(3),     // monster will "become" its summoned leader,
                                // reappearing when that leader is defeated
  MA_HIT_DEGRADE_ARMOR = Fl(4), // monster damages armor
  MA_CAST_SUMMON = Fl(5), // requires that there be one or more summon hordes
                          // with this monster type as the leader
  ...
  MA_TRANSFERENCE = Fl(11), // monster recovers 40 or 90% of the damage that it
                            // inflicts as health
  MA_CAUSES_WEAKNESS =
      Fl(12), // monster attacks cause weakness status in target
  MA_ATTACKS_PENETRATE =
      Fl(13), // monster attacks all adjacent enemies, like an axe
  MA_ATTACKS_ALL_ADJACENT =
      Fl(14), // monster attacks penetrate one layer of enemies, like a spear
  MA_ATTACKS_EXTEND = Fl(15),  // monster attacks from a distance in a cardinal
                               // direction, like a whip
  MA_ATTACKS_STAGGER = Fl(16), // monster attacks will push the player backward
                               // by one space if there is room
  MA_AVOID_CORRIDORS = Fl(17), // monster will avoid corridors when hunting

  SPECIAL_HIT = (MA_HIT_HALLUCINATE | MA_HIT_STEAL_FLEE | MA_HIT_DEGRADE_ARMOR |
                 MA_POISONS | MA_TRANSFERENCE | MA_CAUSES_WEAKNESS |
                 MA_HIT_BURN | MA_ATTACKS_STAGGER),
  ...
  MA_NON_NEGATABLE_ABILITIES = (MA_ATTACKS_PENETRATE | MA_ATTACKS_ALL_ADJACENT |
                                MA_ATTACKS_EXTEND | MA_ATTACKS_STAGGER),
...
enum monsterBookkeepingFlags {
  ...
  MB_TELEPATHICALLY_REVEALED =
      Fl(1), // player can magically see monster and adjacent cells
  MB_PREPLACED =
      Fl(2), // monster dropped onto the level and requires post-processing
  ...
  MB_SUBMERGED = Fl(11), // monster is currently submerged and hence invisible
                         // until it attacks
  MB_JUST_SUMMONED =
      Fl(12),             // used to mark summons so they can be post-processed
  MB_WILL_FLASH = Fl(13), // this monster will flash as soon as control is
                          // returned to the player
  MB_BOUND_TO_LEADER = Fl(14), // monster will die if the leader dies or becomes
                               // separated from the leader
  MB_MARKED_FOR_SACRIFICE = Fl(
      15), // scary glow, monster can be sacrificed in the appropriate machine
  MB_ABSORBING =
      Fl(16), // currently learning a skill by absorbing an enemy corpse
  MB_DOES_NOT_TRACK_LEADER =
      Fl(17),             // monster will not follow its leader around
  MB_IS_FALLING = Fl(18), // monster is plunging downward at the end of the turn
  MB_IS_DYING = Fl(19),   // monster has already been killed and is awaiting the
                          // end-of-turn graveyard sweep (or in purgatory)
  MB_GIVEN_UP_ON_SCENT =
      Fl(20), // to help the monster remember that the scent map is a dead end
  ...
  MB_HAS_ENTRANCED_MOVED =
      Fl(24) // has already moved while entranced and should not move again
};
...
enum creatureModes { MODE_NORMAL, MODE_PERM_FLEEING };
...
typedef struct creature {
  ...
  short weaknessAmount; // number of points of weakness that are inflicted by
                        // the weakness status
  ...
  enum creatureModes
      creatureMode; // current behavioral mode (higher-level than state)

  short mutationIndex; // what mutation the monster has (or -1 for none)

  // Waypoints:
  short targetWaypointIndex; // the index number of the waypoint we're pathing
                             // toward
  boolean waypointAlreadyVisited[MAX_WAYPOINT_COUNT]; // checklist of waypoints
  short lastSeenPlayerAt[2]; // last location at which the monster hunted the
                             // player

  short targetCorpseLoc[2];      // location of the corpse that the monster is
                                 // approaching to gain its abilities
  char targetCorpseName[30];     // name of the deceased monster that we're
                                 // approaching to gain its abilities
  unsigned long absorptionFlags; // ability/behavior flags that the monster will
                                 // gain when absorption is complete
  boolean absorbBehavior; // above flag is behavior instead of ability (ignored
                          // if absorptionBolt is set)
  short absorptionBolt;   // bolt index that the monster will learn to cast when
                          // absorption is complete
  short corpseAbsorptionCounter; // used to measure both the time until the
                                 // monster stops being interested in the
                                 // corpse, and, later, the time until the
                                 // monster finishes absorbing the corpse.
  short **mapToMe;   // if a pack leader, this is a periodically updated pathing
                     // map to get to the leader
  short **safetyMap; // fleeing monsters store their own safety map when out of
                     // player FOV to avoid omniscience
  ...
  short previousHealthPoints; // remembers what your health proportion was at
                              // the start of the turn
  short turnsSpentStationary; // how many (subjective) turns it's been since the
                              // creature moved between tiles
  short flashStrength; // monster will flash soon; this indicates the percent
                       // strength of flash
  color flashColor;    // the color that the monster will flash
  short status[NUMBER_OF_STATUS_EFFECTS];
  short maxStatus[NUMBER_OF_STATUS_EFFECTS]; // used to set the max point on the
                                             // status bars
  unsigned long bookkeepingFlags;
  short spawnDepth;  // keep track of the depth of the machine to which they
                     // relate (for activation monsters)
  short machineHome; // monsters that spawn in a machine keep track of the
                     // machine number here (for activation monsters)
  short xpxp; // exploration experience (used to time telepathic bonding for
              // allies)
  short newPowerCount;   // how many more times this monster can absorb a fallen
                         // monster
  short totalPowerCount; // how many times has the monster been empowered? Used
                         // to recover abilities when negated.

  struct creature *leader;         // only if monster is a follower
  struct creature *carriedMonster; // when vampires turn into bats, one of the
                                   // bats restores the vampire when it dies
...
// these are basically global variables pertaining to the game state and
// player's unique variables:
typedef struct playerCharacter {
  ...
  boolean gameInProgress;   // the game is in progress (the player has not died,
                            // won or quit yet)
  ...
  boolean
      automationActive; // cut some corners during redraws to speed things up
  ...
  boolean cautiousMode; // used to prevent careless deaths caused by holding
                        // down a key
  boolean receivedLevitationWarning; // only warn you once when you're hovering
                                     // dangerously over liquid
  boolean
      updatedSafetyMapThisTurn; // so it's updated no more than once per turn
  boolean updatedAllySafetyMapThisTurn; // so it's updated no more than once per
                                        // turn
  boolean updatedMapToSafeTerrainThisTurn; // so it's updated no more than once
                                           // per turn
  boolean
      updatedMapToShoreThisTurn; // so it's updated no more than once per turn
  boolean easyMode;              // enables easy mode
  boolean inWater;               // helps with the blue water filter effect
  boolean heardCombatThisTurn;   // so you get only one "you hear combat in the
                                 // distance" per turn
  boolean creaturesWillFlashThisTurn; // there are creatures out there that need
                                      // to flash before the turn ends
  ...
  boolean eligibleToUseStairs;   // so the player uses stairs only when he steps
                                 // onto them
  ...
  unsigned long goldGenerated; // how much gold has been generated on the
                               // levels, not counting gold held by monsters
  short strength;
  unsigned short
      monsterSpawnFuse; // how much longer till a random monster spawns
  ...
  short ticksTillUpdateEnvironment; // so that some periodic things happen in
                                    // objective time
  unsigned short scentTurnNumber;   // helps make scent-casting work
  unsigned long playerTurnNumber;   // number of input turns in recording. Does
                                    // not increment during paralysis.
  unsigned long absoluteTurnNumber; // number of turns since the beginning of
                                    // time. Always increments.
  signed long milliseconds; // milliseconds since launch, to decide whether to
                            // engage cautious mode
  ...
  short previousPoisonPercent; // and your poison proportion, to display
                               // percentage alerts for each.
  ...
  creature *lastTarget; // to keep track of the last monster the player has
                        // thrown at or zapped
  item *lastItemThrown;
  ...
  short sidebarLocationList[ROWS * 2][2]; // to keep track of which location
                                          // each line of the sidebar references
  ...
  unsigned short
      patchVersion;       // what patch version of the game this was recorded on
  ...
  short
      playbackDelayPerTurn; // base playback speed; modified per turn by events
  short playbackDelayThisTurn; // playback speed as modified
  boolean playbackPaused;
  boolean playbackFastForward;  // for loading saved games and such -- disables
                                // drawing and prevents pauses
  ...
  boolean playbackBetweenTurns; // i.e. waiting for a top-level input -- iff,
                                // permit playback commands
  unsigned long nextAnnotationTurn; // the turn number during which to display
                                    // the next annotation
  char nextAnnotation[5000];        // the next annotation
  unsigned long
      locationInAnnotationFile; // how far we've read in the annotations file
  ...
  // What do you want to do, player -- play, play with seed, resume, recording,
  // high scores or quit?
...
enum machineFeatureFlags {
  MF_GENERATE_ITEM = Fl(0), // feature entails generating an item (overridden if
                            // the machine is adopting an item)
  MF_OUTSOURCE_ITEM_TO_MACHINE =
      Fl(1),                  // item must be adopted by another machine
  MF_BUILD_VESTIBULE = Fl(2), // call this at the origin of a door room to
                              // create a new door guard machine there
  MF_ADOPT_ITEM = Fl(3), // this feature will take the adopted item (be it from
                         // another machine or a previous feature)
  MF_NO_THROWING_WEAPONS =
      Fl(4), // the generated item cannot be a throwing weapon
  MF_GENERATE_HORDE =
      Fl(5), // generate a monster horde that has all of the horde flags
  MF_BUILD_AT_ORIGIN = Fl(6), // generate this feature at the room entrance
  // unused                       = Fl(7),    //
  MF_PERMIT_BLOCKING = Fl(8), // permit the feature to block the map's
                              // passability (e.g. to add a locked door)
  MF_TREAT_AS_BLOCKING =
      Fl(9), // treat this terrain as though it blocks, for purposes of deciding
             // whether it can be placed there
  MF_NEAR_ORIGIN = Fl(10), // feature must spawn in the rough quarter of tiles
                           // closest to the origin
  MF_FAR_FROM_ORIGIN = Fl(11), // feature must spawn in the rough quarter of
                               // tiles farthest from the origin
  MF_MONSTER_TAKE_ITEM =
      Fl(12), // the item associated with this feature (including if adopted)
              // will be in possession of the horde leader that's generated
  MF_MONSTER_SLEEPING = Fl(13), // the monsters should be asleep when generated
  MF_MONSTER_FLEEING =
      Fl(14), // the monsters should be permanently fleeing when generated
  MF_EVERYWHERE = Fl(
      15), // generate the feature on every tile of the machine (e.g. carpeting)
  MF_ALTERNATIVE = Fl(16), // build only one feature that has this flag per
                           // machine; the rest are skipped
  MF_ALTERNATIVE_2 =
      Fl(17), // same as MF_ALTERNATIVE, but provides for a second set of
              // alternatives of which only one will be chosen
  MF_REQUIRE_GOOD_RUNIC = Fl(18), // generated item must be uncursed runic
  MF_MONSTERS_DORMANT =
      Fl(19), // monsters are dormant, and appear when a dungeon feature with
              // DFF_ACTIVATE_DORMANT_MONSTER spawns on their tile
  // unused                       = Fl(20),   //
  MF_BUILD_IN_WALLS =
      Fl(21), // build in an impassable tile that is adjacent to the interior
  MF_BUILD_ANYWHERE_ON_LEVEL =
      Fl(22), // build anywhere on the level that is not inside the machine
  MF_REPEAT_UNTIL_NO_PROGRESS =
      Fl(23), // keep trying to build this feature set until no changes are made
  MF_IMPREGNABLE =
      Fl(24), // this feature's location will be immune to tunneling
  MF_IN_VIEW_OF_ORIGIN = Fl(25), // this feature must be in view of the origin
  MF_IN_PASSABLE_VIEW_OF_ORIGIN =
      Fl(26), // this feature must be in view of the origin, where "view" is
              // blocked by pathing blockers
  MF_NOT_IN_HALLWAY =
      Fl(27), // the feature location must have a passableArcCount of <= 1
  MF_NOT_ON_LEVEL_PERIMETER =
      Fl(28), // don't build it in the outermost walls of the level
  MF_SKELETON_KEY = Fl(29), // if a key is generated or adopted by this feature,
                            // it will open all locks in this machine.
  MF_KEY_DISPOSABLE =
      Fl(30), // if a key is generated or adopted, it will self-destruct after
              // being used at this current location.
};

typedef struct machineFeature {
  // terrain
  enum dungeonFeatureTypes
      featureDF; // generate this DF at the feature location (0 for none)
  enum tileType terrain; // generate this terrain tile at the feature location
                         // (0 for none)
  enum dungeonLayers layer; // generate the terrain tile in this layer

  short
      instanceCountRange[2]; // generate this range of instances of this feature
  ...
  short monsterID; // generate a monster of this kind if MF_GENERATE_MONSTER is
                   // set

  short personalSpace; // subsequent features must be generated more than this
                       // many tiles away from this feature
...
enum blueprintFlags {
  ...
  BP_VESTIBULE = Fl(1),  // spawns in a doorway (location must be given) and
                         // expands outward, to guard the room
  BP_PURGE_PATHING_BLOCKERS =
      Fl(2),                 // clean out traps and other T_PATHING_BLOCKERs
  BP_PURGE_INTERIOR = Fl(3), // clean out all of the terrain in the interior
                             // before generating the machine
  BP_PURGE_LIQUIDS = Fl(4),  // clean out all of the liquids in the interior
                             // before generating the machine
  BP_SURROUND_WITH_WALLS =
      Fl(5), // fill in any impassable gaps in the perimeter (e.g. water, lava,
             // brimstone, traps) with wall
  BP_IMPREGNABLE = Fl(6), // impassable perimeter and interior tiles are locked;
                          // tunneling bolts will bounce off harmlessly
  BP_REWARD = Fl(7),      // metered reward machines
  BP_OPEN_INTERIOR =
      Fl(8), // clear out walls in the interior, widen the interior until convex
             // or bumps into surrounding areas
  BP_MAXIMIZE_INTERIOR =
      Fl(9), // same as BP_OPEN_INTERIOR but expands the room as far as it can
             // go, potentially surrounding the whole level.
  BP_ROOM =
      Fl(10), // spawns in a dead-end room that is dominated by a chokepoint of
              // the given size (as opposed to a random place of the given size)
  BP_TREAT_AS_BLOCKING =
      Fl(11), // abort the machine if, were it filled with wall tiles, it would
              // disrupt the level connectivity
  BP_REQUIRE_BLOCKING =
      Fl(12), // abort the machine unless, were it filled with wall tiles, it
              // would disrupt the level connectivity
  BP_NO_INTERIOR_FLAG =
      Fl(13), // don't flag the area as being part of a machine
  BP_REDESIGN_INTERIOR =
      Fl(14), // nuke and pave -- delete all terrain in the interior and build
              // entirely new rooms within the bounds
};
...
typedef struct blueprint {
  ...
  short frequency; // frequency (number of tickets this blueprint enters in the
                   // blueprint selection raffle)
  short featureCount; // how many different types of features follow (max of 20)
  short dungeonProfileType;   // if BP_REDESIGN_INTERIOR is set, which dungeon
                              // profile do we use?
...
typedef struct autoGenerator {
  ...
  enum machineTypes machine; // Machine placement also respects BP_ placement
                             // flags in the machine blueprint
...
typedef struct brogueButton {
  ...
  color buttonColor; // background of the button; further gradient-ized when
                     // displayed
  short opacity;     // further reduced by 50% if not enabled
  enum displayGlyph
      symbol[COLS]; // Automatically replace the nth asterisk in the button
                    // label text with the nth character supplied here, if one
                    // is given. (Primarily to display magic character and item
                    // symbols in the inventory display.)
  unsigned long flags;
} brogueButton;
...
void normColor(color *baseColor, const short aggregateMultiplier,
               const short colorTranslation);
void getCellAppearance(short x, short y, enum displayGlyph *returnChar,
                       color *returnForeColor, color *returnBackColor);
...
boolean buildAMachine(enum machineTypes bp, short originX, short originY,
                      unsigned long requiredMachineFlags, item *adoptiveItem,
                      item *parentSpawnedItems[50],
                      creature *parentSpawnedMonsters[50]);
void attachRooms(short **grid, const dungeonProfile *theDP, short attempts,
                 short maxRoomCount);
void digDungeon();
void updateMapToShore();
short levelIsDisconnectedWithBlockingMap(char blockingMap[DCOLS][DROWS],
                                         boolean countRegionSize);
void resetDFMessageEligibility();
boolean fillSpawnMap(enum dungeonLayers layer, enum tileType surfaceTileType,
                     char spawnMap[DCOLS][DROWS], boolean blockedByOtherLayers,
                     boolean refresh, boolean superpriority);
boolean spawnDungeonFeature(short x, short y, dungeonFeature *feat,
                            boolean refreshCell, boolean abortIfBlocking);
...
void plotChar(enum displayGlyph inputChar, short xLoc, short yLoc,
              short backRed, short backGreen, short backBlue, short foreRed,
              short foreGreen, short foreBlue);
boolean pauseForMilliseconds(short milliseconds);
boolean isApplicationActive();
void nextKeyOrMouseEvent(rogueEvent *returnEvent, boolean textInput,
                         boolean colorsDance);
void notifyEvent(short eventId, int data1, int data2, const char *str1,
                 const char *str2);
...
void refreshSideBar(short focusX, short focusY,
                    boolean focusedEntityMustGoFirst);
void printHelpScreen();
void printDiscoveriesScreen();
void printHighScores(boolean hiliteMostRecent);
void displayGrid(short **map);
void printSeed();
void printProgressBar(short x, short y, const char barLabel[COLS],
                      long amtFilled, long amtMax, color *fillColor,
                      boolean dim);
...
short printTerrainInfo(short x, short y, short py, const char *description,
                       boolean dim, boolean highlight);
void rectangularShading(short x, short y, short width, short height,
                        const color *backColor, short opacity,
                        cellDisplayBuffer dbuf[COLS][ROWS]);
short printTextBox(char *textBuf, short x, short y, short width,
                   color *foreColor, color *backColor,
                   cellDisplayBuffer rbuf[COLS][ROWS], brogueButton *buttons,
                   short buttonCount);
...
unsigned long printCarriedItemDetails(item *theItem, short x, short y,
                                      short width, boolean includeButtons,
                                      cellDisplayBuffer rbuf[COLS][ROWS]);
void funkyFade(cellDisplayBuffer displayBuf[COLS][ROWS],
               const color *colorStart, const color *colorEnd, short stepCount,
               short x, short y, boolean invert);
void displayCenteredAlert(char *message);
void flashMessage(char *message, short x, short y, int time, color *fColor,
                  color *bColor);
...
void applyColorAverage(color *baseColor, const color *newColor,
                       short averageWeight);
void applyColorAugment(color *baseColor, const color *augmentingColor,
                       short augmentWeight);
...
void irisFadeBetweenBuffers(cellDisplayBuffer fromBuf[COLS][ROWS],
                            cellDisplayBuffer toBuf[COLS][ROWS], short x,
                            short y, short frameCount, boolean outsideIn);
void colorBlendCell(short x, short y, color *hiliteColor, short hiliteStrength);
void hiliteCell(short x, short y, const color *hiliteColor,
                short hiliteStrength, boolean distinctColors);
void colorMultiplierFromDungeonLight(short x, short y, color *editColor);
void plotCharWithColor(enum displayGlyph inputChar, short xLoc, short yLoc,
                       const color *cellForeColor, const color *cellBackColor);
void plotCharToBuffer(enum displayGlyph inputChar, short x, short y,
                      color *foreColor, color *backColor,
                      cellDisplayBuffer dbuf[COLS][ROWS]);
void plotForegroundChar(enum displayGlyph inputChar, short x, short y,
                        color *foreColor, boolean affectedByLighting);
void commitDraws();
void dumpLevelToScreen();
void hiliteCharGrid(char hiliteCharGrid[DCOLS][DROWS], color *hiliteColor,
                    short hiliteStrength);
void blackOutScreen();
void colorOverDungeon(const color *color);
void copyDisplayBuffer(cellDisplayBuffer toBuf[COLS][ROWS],
                       cellDisplayBuffer fromBuf[COLS][ROWS]);
void clearDisplayBuffer(cellDisplayBuffer dbuf[COLS][ROWS]);
color colorFromComponents(char rgb[3]);
void overlayDisplayBuffer(cellDisplayBuffer overBuf[COLS][ROWS],
                          cellDisplayBuffer previousBuf[COLS][ROWS]);
void flashForeground(short *x, short *y, color **flashColor,
                     short *flashStrength, short count, short frames);
...
void colorFlash(const color *theColor, unsigned long reqTerrainFlags,
                unsigned long reqTileFlags, short frames, short maxRadius,
                short x, short y);
void printString(const char *theString, short x, short y, color *foreColor,
                 color *backColor, cellDisplayBuffer dbuf[COLS][ROWS]);
short wrapText(char *to, const char *sourceText, short width);
short printStringWithWrapping(char *theString, short x, short y, short width,
                              color *foreColor, color *backColor,
                              cellDisplayBuffer dbuf[COLS][ROWS]);
boolean getInputTextString(char *inputText, const char *prompt, short maxLength,
                           const char *defaultEntry, const char *promptSuffix,
                           short textEntryType, boolean useDialogBox);
void displayChokeMap();
void displayLoops();
boolean pauseBrogue(short milliseconds);
void nextBrogueEvent(rogueEvent *returnEvent, boolean textInput,
                     boolean colorsDance, boolean realInputEvenInPlayback);
...
void emptyGraveyard();
void freeEverything();
boolean randomMatchingLocation(short *x, short *y, short dungeonType,
                               short liquidType, short terrainType);
...
void describedItemBasedOnParameters(short theCategory, short theKind,
                                    short theQuantity, short theItemOriginDepth,
                                    char *buf);
...
boolean handleWhipAttacks(creature *attacker, enum directions dir,
                          boolean *aborted);
boolean handleSpearAttacks(creature *attacker, enum directions dir,
                           boolean *aborted);
boolean diagonalBlocked(const short x1, const short y1, const short x2,
                        const short y2, const boolean limitToPlayerKnowledge);
boolean playerMoves(short direction);
void calculateDistances(short **distanceMap, short destinationX,
                        short destinationY, unsigned long blockingTerrainFlags,
                        creature *traveler, boolean canUseSecretDoors,
                        boolean eightWays);
short pathingDistance(short x1, short y1, short x2, short y2,
                      unsigned long blockingTerrainFlags);
short nextStep(short **distanceMap, short x, short y, creature *monst,
               boolean reverseDirections);
...
void getLocationFlags(const short x, const short y, unsigned long *tFlags,
                      unsigned long *TMFlags, unsigned long *cellFlags,
...
short getPlayerPathOnMap(short path[1000][2], short **map, short originX,
                         short originY);
...
short randValidDirectionFrom(creature *monst, short x, short y,
                             boolean respectAvoidancePreferences);
...
void messageWithColor(char *msg, color *theColor,
                      boolean requireAcknowledgment);
...
void updateFieldOfView(short xLoc, short yLoc, short radius, boolean paintScent,
                       boolean passThroughCreatures, boolean setFieldOfView,
                       short theColor[3], short fadeToPercent);
void betweenOctant1andN(short *x, short *y, short x0, short y0, short n);

void getFOVMask(char grid[DCOLS][DROWS], short xLoc, short yLoc,
                fixpt maxRadius, unsigned long forbiddenTerrain,
                unsigned long forbiddenFlags, boolean cautiousOnWalls);
void scanOctantFOV(char grid[DCOLS][DROWS], short xLoc, short yLoc,
                   short octant, fixpt maxRadius, short columnsRightFromOrigin,
                   long startSlope, long endSlope,
                   unsigned long forbiddenTerrain, unsigned long forbiddenFlags,
                   boolean cautiousOnWalls);

creature *generateMonster(short monsterID, boolean itemPossible,
                          boolean mutationPossible);
short chooseMonster(short forLevel);
creature *spawnHorde(short hordeID, short x, short y,
                     unsigned long forbiddenFlags, unsigned long requiredFlags);
void fadeInMonster(creature *monst);
boolean removeMonsterFromChain(creature *monst, creature *theChain);
boolean monsterWillAttackTarget(const creature *attacker,
...
short pickHordeType(short depth, enum monsterTypes summonerType,
                    unsigned long forbiddenFlags, unsigned long requiredFlags);
...
boolean monsterBlinkToPreferenceMap(creature *monst, short **preferenceMap,
                                    boolean blinkUphill);
...
short runicWeaponChance(item *theItem, boolean customEnchantLevel,
                        fixpt enchantLevel);
void magicWeaponHit(creature *defender, item *theItem, boolean backstabbed);
void teleport(creature *monst, short x, short y,
              boolean respectTerrainAvoidancePreferences);
...
boolean monsterHiddenBySubmersion(const creature *monst,
...
void applyArmorRunicEffect(char returnString[DCOLS], creature *attacker,
...
boolean inflictDamage(creature *attacker, creature *defender, short damage,
                      const color *flashColor, boolean ignoresProtectionShield);
void addPoison(creature *monst, short totalDamage,
               short concentrationIncrement);
void killCreature(creature *decedent, boolean administrativeDeath);
void buildHitList(creature **hitList, const creature *attacker,
...
void aggravateMonsters(short distance, short x, short y,
                       const color *flashColor);
short getLineCoordinates(short listOfCoordinates[][2], const short originLoc[2],
                         const short targetLoc[2]);
void getImpactLoc(short returnLoc[2], const short originLoc[2],
                  const short targetLoc[2], const short maxDistance,
                  const boolean returnLastEmptySpace);
...
short reflectBolt(short targetX, short targetY, short listOfCoordinates[][2],
...
boolean zap(short originLoc[2], short targetLoc[2], bolt *theBolt,
            boolean hideDetails);
boolean nextTargetAfter(short *returnX, short *returnY, short targetX,
                        short targetY, boolean targetEnemies,
                        boolean targetAllies, boolean targetItems,
                        boolean targetTerrain, boolean requireOpenPath,
                        boolean reverseDirection);
boolean moveCursor(boolean *targetConfirmed, boolean *canceled, boolean *tabKey,
                   short targetLoc[2], rogueEvent *event, buttonState *state,
                   boolean colorsDance, boolean keysMoveCursor,
...
void itemName(item *theItem, char *root, boolean includeDetails,
              boolean includeArticle, color *baseColor);
char displayInventory(unsigned short categoryMask, unsigned long requiredFlags,
                      unsigned long forbiddenFlags, boolean waitForAcknowledge,
...
boolean getQualifyingLocNear(short loc[2], short x, short y,
                             boolean hallwaysAllowed,
                             char blockingMap[DCOLS][DROWS],
                             unsigned long forbiddenTerrainFlags,
                             unsigned long forbiddenMapFlags,
                             boolean forbidLiquid, boolean deterministic);
boolean getQualifyingGridLocNear(short loc[2], short x, short y,
...
void findReplaceGrid(short **grid, short findValueMin, short findValueMax,
                     short fillValue);
short floodFillGrid(short **grid, short x, short y, short eligibleValueMin,
                    short eligibleValueMax, short fillValue);
void drawRectangleOnGrid(short **grid, short x, short y, short width,
                         short height, short value);
void drawCircleOnGrid(short **grid, short x, short y, short radius,
                      short value);
void getTerrainGrid(short **grid, short value, unsigned long terrainFlags,
...
boolean getQualifyingPathLocNear(short *retValX, short *retValY, short x,
                                 short y, boolean hallwaysAllowed,
                                 unsigned long blockingTerrainFlags,
                                 unsigned long blockingMapFlags,
                                 unsigned long forbiddenTerrainFlags,
                                 unsigned long forbiddenMapFlags,
                                 boolean deterministic);
void createBlobOnGrid(short **grid, short *retMinX, short *retMinY,
                      short *retWidth, short *retHeight, short roundCount,
                      short minBlobWidth, short minBlobHeight,
                      short maxBlobWidth, short maxBlobHeight,
                      short percentSeeded, char birthParameters[9],
                      char survivalParameters[9]);
...
void describeMonsterClass(char *buf, const short classID,
...
item *promptForItemOfType(unsigned short category, unsigned long requiredFlags,
                          unsigned long forbiddenFlags, char *prompt,
...
boolean paintLight(lightSource *theLight, short x, short y,
                   boolean isMinersLight, boolean maintainShadows);
void backUpLighting(short lights[DCOLS][DROWS][3]);
void restoreLighting(short lights[DCOLS][DROWS][3]);
void updateLighting();
boolean playerInDarkness();
flare *newFlare(lightSource *light, short x, short y, short changePerFrame,
                short limit);
...
void getAvailableFilePath(char *filePath, const char *defaultPath,
                          const char *suffix);
...
void printSeedCatalog(unsigned long startingSeed,
                      unsigned long numberOfSeedsToScan,
                      unsigned int scanThroughDepth, boolean isCsvFormat);

void initializeButton(brogueButton *button);
void drawButtonsInState(buttonState *state);
void initializeButtonState(buttonState *state, brogueButton *buttons,
                           short buttonCount, short winX, short winY,
                           short winWidth, short winHeight);
short processButtonInput(buttonState *state, boolean *canceled,
                         rogueEvent *event);
short smoothHiliteGradient(const short currentXValue, const short maxXValue);
void drawButton(brogueButton *button, enum buttonDrawStates highlight,
                cellDisplayBuffer dbuf[COLS][ROWS]);
short buttonInputLoop(brogueButton *buttons, short buttonCount, short winX,
                      short winY, short winWidth, short winHeight,
